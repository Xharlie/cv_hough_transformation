...
signAcademicHonestyPolicy('Qiangeng Xu', 'qx2128');
...
% hough_threshold = [255];
img_list = {'hough_1', 'hough_2', 'hough_3'};
hough_threshold = [89 75 50];
local_max_diameter = [[13,13];[20,20];[15,15]];

for i = 1:length(img_list)
    orig_img = imread([img_list{i} '.png']);
    hough_img = imread(['accumulator_' img_list{i} '.png']);
    line_img = lineFinder(orig_img, hough_img, hough_threshold(i),local_max_diameter(i,:));
